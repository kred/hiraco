...
3. **Optical Base Recovery (Wiener Deconvolution)**: Because 8+ physical sub-frames inevitably undergo sensor-shift overlap, lens diffraction, and micro-vibrations, the intrinsic image is mathematically blurred. We utilize `FFTW` (Fast Fourier Transform) to apply a Wiener Deconvolution filter. Crucially, the mathematical model accounts for the true **physical photodiode integration**: because the High-Res sensor shifts by half-pixels to capture data, the virtual photodiode bucket is a 2x oversized 2D box. By incorporating this box integration model (a `sinc` limit in the frequency domain) alongside the optical Gaussian lens blur, the deconvolution engine naturally decays its amplification exactly at the Nyquist resolving boundary, flawlessly rebuilding structural limits without arbitrarily amplifying Bayer-derived tracking artifacts.
...
- The optical blurring property is modeled dynamically. It accurately multiplies the smooth Gaussian spread of the camera optics by the hard `sinc(x)` frequency envelope generated by the physical pixel binning (a 2x oversized uniform box derived from the half-pixel shifts). 
- Because the `sinc` limit mathematically reaches absolute zero at the Nyquist frequency limit, the algorithm safely cuts off unmanageable inverse amplifications near sub-pixel misalignment limits.
- The isolated frequencies are linearly restored to image matrices (Inverse FFT).

*Why this works cleanly*: Typical deconvolutions mathematically assume standard single-shot point functions. A generic Gaussian inverse function will erroneously ask for a 500%+ high-frequency amplification exactly where High-Res composites naturally house tiny Bayer tracking artifacts. This commonly forces severe "maze" or "cross-hatch" stippling artifacts across fine structural edges. However, by strictly adopting the correct sensor physical integration limit (box-leveling), `hiraco` effortlessly avoids multiplying the artifacts while safely tightening true macro structures.
